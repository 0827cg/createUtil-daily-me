...
class CopyRename:
    ...
    def newTraverDir(self, strDir, intRunNum, intTotalFileNum):
        ...
        # 遍历文件
        # 目的: 将不足的文件多次拷贝使得达到数目
        # 例如, 需要5000张图片, 从28张图片中进行复制, 尽量使得每张图片的复制次数相近
        # 这里得到的新的图片, 将以0~5000的名字来命名, 存放到同一文件夹中

        # strDir: 需要遍历的文件目录,全路据
        # intRunNum: 执行的第几次, 当然一般从1开始
        # intTotalFileNum: strDir中所有的文件个数
        # --modify time: 2018-07-26 11:03
            ...
            if intRunNum <= intNeedRun:
                        ...
                        for strFileName in listFileName:
                            ...
                            if self.intFileName < (self.intMaxDirNum * self.intMaxDirItemNum):
                                ...
                                self.logUtil.writerLog('-->执行第' + str(intRunNum) + '次拷贝, intHasCopyNumEvery='
                                                       + str(intHasCopyNumEvery))
                                self.copyFile(strFilePath, strSaveFilePath, strFileName)
                                self.logUtil.writerLog(str(intRunNum) + '-' + str(intHasCopyNumEvery) + '-->'
                                ...
                                self.logUtil.writerLog('已拷贝的图片总数已达到要求, self.intFileName = ' +
                                                       str(self.intFileName), 'intRunNum = ' + str(intRunNum))
                                self.logUtil.writerLog('程序将退出')
                                break
